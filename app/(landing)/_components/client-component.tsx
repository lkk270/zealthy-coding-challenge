...
const ClientComponent = ({ config }: ClientComponentProps) => {
	...
	const stepConfigs = config.reduce((acc: Record<string, StepConfig>, curr) => {
		...
		if (!acc[step]) {
			...
			acc[step].schema = createStepSchema([
				...acc[step].components,
				curr.component,
			]);
		}
	...
	const [currentStep, setCurrentStep] = useState(() => {
		...
		const currentStepIndex = sortedSteps.findIndex(
			(config) => config.step === user.currentStep
		);
	...
	const handleNext = async (data?: Record<string, unknown>) => {
		...
		const nextStepLabel = (
			nextStep >= sortedSteps.length ? "Finished" : sortedSteps[nextStep].step
		) as StepLabel;
	...
	const handlePrevious = () => {
		...
		startTransition(async () => {
			const result = await updateUserStep(
				user.id,
				previousStepLabel as StepLabel,
				{}
			);
