...
export const DynamicForm = ({
	...
	const componentRenderers: Record<
		string,
		(props: ComponentRendererProps) => React.ReactElement
	> = {
	...
	const handleSubmit = async () => {
		...
		const relevantData = components.reduce(
			(acc: Partial<FormValues>, component) => {
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
				{components.map((component) => {
					const key = component.toLowerCase().replace(/\s+/g, "");
					const renderer =
						componentRenderers[key as keyof typeof componentRenderers];
					return renderer ? (
						<div key={key}>
							{renderer({
