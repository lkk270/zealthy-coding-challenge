...
export async function changePassword(userId: string, values: z.infer<typeof PasswordChangeSchema>) {
  ...
  try {
    ...
    await db.update(users).set({ password: newPassword }).where(eq(users.id, existingUser.id));
