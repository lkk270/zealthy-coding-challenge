...
export function ConfigurationEditor({ initialConfig }: ConfigurationEditorProps) {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const [moved] = newSteps[sourceList as keyof typeof steps].splice(source.index, 1);
    newSteps[destList as keyof typeof steps].splice(destination.index, 0, moved);
    ...
    <>
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="grid grid-cols-2 gap-8">
          <StepContainer title="Step 2" droppableId="step2" components={steps.step2} />
          <StepContainer title="Step 3" droppableId="step3" components={steps.step3} />
        </div>
      </DragDropContext>
